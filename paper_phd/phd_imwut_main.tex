...
Five diagnostics anchor Table~\ref{tab:eval-pred}: the empirical
CDF that produces the $\le 1.5$\,m and $\le 3$\,m columns, the
signed-error PDF that checks for bias, the reliability plot that
inspects conformal calibration directly, the error-versus-duration
scatter that exposes which rides drive the long tail, and the
per-experiment view that locates the outliers.
...
Figure~\ref{fig:eval-pred-reliability} sweeps the nominal
coverage level from $50\,\%$ to $99\,\%$ and plots the empirical
coverage at each.  A perfectly calibrated predictor sits on the
$y\!=\!x$ diagonal; the trapezoid pipeline tracks the diagonal
within a few points across the whole range.

\begin{figure}[H]
\centering
\includegraphics[width=0.65\linewidth]{pred_reliability.png}
\caption{Reliability curve: empirical coverage of the trapezoid
  pipeline against nominal coverage from $50\,\%$ to $99\,\%$.
  The conformal calibration of~\S\ref{sec:algo-ci} holds across
  the whole sweep, not only at the deployed $90\,\%$ point.}
\label{fig:eval-pred-reliability}
\end{figure}

\paragraph{Error versus ride duration.}
Long rides accumulate more integration noise but also more
acceleration-energy signal; the two effects partly cancel.
Figure~\ref{fig:eval-pred-err-dur} plots absolute error against
GT ride duration.  Short rides ($<5$\,s) carry the heaviest
relative error because the trapezoid lobes are sparsely sampled;
longer rides cluster near the floor of the integration noise.

\begin{figure}[H]
\centering
\includegraphics[width=0.65\linewidth]{pred_err_vs_duration.png}
\caption{Absolute $\Delta h$ error against GT ride duration for
  the trapezoid fit on clean rides.  The conformal interval
  widens on short rides exactly where the per-ride
  $\sigma$~of~\eqref{eq:sigma-dh} grows, which is the adaptive
  promise of~\S\ref{sec:algo-ci}.}
\label{fig:eval-pred-err-dur}
\end{figure}

\paragraph{Coverage by ride duration.}
Figure~\ref{fig:eval-pred-cov-dur} bins the clean rides by
duration and reports the realised coverage in each bin.  Coverage
holds at or above the nominal $90\,\%$ across every bin --- the
interval widens on short rides rather than mis-covering them.

\begin{figure}[H]
\centering
\includegraphics[width=0.65\linewidth]{pred_cov_by_duration.png}
\caption{Empirical coverage by ride-duration bin for the
  trapezoid pipeline.  Coverage holds at or above the nominal
  $90\,\%$ on every bin; short rides keep coverage by widening
  the interval, not by mis-covering.}
\label{fig:eval-pred-cov-dur}
\end{figure}

\paragraph{Per-experiment view.}
Figure~\ref{fig:eval-pred-per-exp} disaggregates the prediction
error by experiment.  The three damped-recording sessions of
\S\ref{sec:discussion} sit as tall bars; every other recording
sits well below the one-floor target.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{pred_per_exp.png}
\caption{Per-experiment MAE for the trapezoid pipeline on clean
  rides.  The few tall bars are the damped recordings the quality
  filter (Table~\ref{tab:eval-pipeline}) rejects; the bulk of the
  experiments sit comfortably below the $\pm 1.5$\,m floor
  target.}
\label{fig:eval-pred-per-exp}
\end{figure}
