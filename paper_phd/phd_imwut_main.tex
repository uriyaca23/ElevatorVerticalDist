...
shared-shape joint score of~\S\ref{sec:algo-seg}, and explains every
calibrated threshold of Table~\ref{tab:seg-thresholds}.
...
\paragraph{Per-threshold explanations.}
\label{app:thresholds}
The gate equations fix the \emph{form} of each test; the rest of this
appendix walks Table~\ref{tab:seg-thresholds} row by row and explains
what each calibrated \emph{value} rejects.

\paragraph{Detection shape and amplitude floors.}
The first two rows reject samples that cannot be a take-off or
...
\paragraph{Peak de-duplication and the same-sign gap.}
The next two rows stop one physical lobe from entering the pair stage
...
\paragraph{Gate 1 --- the pairing window $[t_{\min},t_{\max}]$.}
...
\paragraph{Gate 2 --- the shared-shape joint score $r_{\mathrm{pair}}$.}
...
\paragraph{Gate 3 --- the shared amplitude $a_{\mathrm{pair}}$.}
...
\paragraph{Gate 4 --- grid energy $e_{\min}$.}
...
\paragraph{Gate 5 --- the quiet-middle cruise gate $\rho$.}
...
\paragraph{Gate 6 --- the duration penalty $\lambda$ and greedy resolution.}
