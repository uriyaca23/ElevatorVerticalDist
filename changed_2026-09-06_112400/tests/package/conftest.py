...
def make_subthreshold_ride_acc(dur_s: float = 80.0) -> pd.DataFrame:
    """A trace with one detected ride plus one *undetected* pulse pair.

    The second pair's amplitude (0.25 m/s²) clears the per-peak floor
    (``min_peak_abs_a`` = 0.20) but not the pair floor
    (``min_pair_abs_a`` = 0.30), so the detector proposes no ride around
    t = 55-65 s. A window there therefore drives
    ``segmentor._detail_from_state`` down its ``_fit_fresh`` branch - the
    one path :func:`findSegmentParameters` cannot reach via a detected ride.
    """
    n = int(dur_s * FS)
    t = np.arange(n) / FS
    rng = np.random.default_rng(3)
    ride = (trapezoid_pulse(t, 20.0, 1.2, 0.5, +1.0)
            + trapezoid_pulse(t, 32.0, 1.2, 0.5, -1.0)
            + trapezoid_pulse(t, 55.0, 1.2, 0.5, +0.25)
            + trapezoid_pulse(t, 65.0, 1.2, 0.5, -0.25))
    return pd.DataFrame({
        "timestamp_ms": (t * 1000).astype(np.int64) + T0_MS,
        "x": rng.normal(0.0, 0.01, n),
        "y": rng.normal(0.0, 0.01, n),
        "z": 9.81 + ride + rng.normal(0.0, 0.01, n),
    })
...
@pytest.fixture(scope="session")
def subthreshold_ride_acc() -> pd.DataFrame:
    return make_subthreshold_ride_acc()
