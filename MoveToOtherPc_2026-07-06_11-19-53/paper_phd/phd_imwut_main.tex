...
\subsection{The smartphone inertial sensors}
\label{sec:bg-imu}
...
\begin{wrapfigure}{r}{0.40\linewidth}
  ...
  \begin{tikzpicture}[>=Stealth, font=\scriptsize, scale=1.0]
    \coordinate (O) at (0,0);
    % --- world (earth) frame, isometric ---
    \draw[->, gray!55, line width=0.5pt] (O) -- (0,2.15)
          node[above, black!75, inner sep=1pt] {$\hat z_w$};
    \draw[->, gray!55, line width=0.5pt] (O) -- (1.28,-0.44)
          node[right, black!75, inner sep=1pt] {$\hat x_w$};
    \draw[->, gray!55, line width=0.5pt] (O) -- (-1.00,-0.40)
          node[left, black!75, inner sep=1pt] {$\hat y_w$};
    % --- tilted phone: 3D slab ---
    \fill[blue!20, draw=blue!55!black, line width=0.45pt]      % front rim
        (0.02,0.66) -- (1.05,0.40) -- (1.05,0.28) -- (0.02,0.54) -- cycle;
    \fill[blue!15, draw=blue!55!black, line width=0.45pt]      % right rim
        (1.05,0.40) -- (1.28,0.98) -- (1.28,0.86) -- (1.05,0.28) -- cycle;
    \fill[blue!10, draw=blue!55!black, line width=0.55pt]      % top face
        (0.02,0.66) -- (1.05,0.40) -- (1.28,0.98) -- (0.25,1.24) -- cycle;
    \coordinate (Pc) at (0.64,0.82);
    % body axes on the case
    \draw[->, blue!45!black, line width=0.5pt] (Pc) -- ++(0.42,-0.10)
          node[right, inner sep=0.5pt] {$x$};
    \draw[->, blue!45!black, line width=0.5pt] (Pc) -- ++(0.16,0.40)
          node[above, inner sep=0.5pt] {$y$};
    \draw[->, blue!45!black, line width=0.5pt] (Pc) -- ++(-0.30,0.20)
          node[left, inner sep=0.5pt] {$z$};
    % measured acceleration (body frame) -- what the phone records
    \draw[->, red!75!black, line width=1pt] (Pc) -- (1.04,1.80);
    \node[red!75!black, inner sep=1pt] at (1.16,1.88) {$\mathbf a$};
    % wanted acceleration (earth frame) -- vertical
    \draw[->, teal!50!black, line width=1pt] (Pc) -- (0.64,1.90);
    \node[teal!45!black, left, inner sep=1pt] at (0.60,1.86) {$\mathbf a^{\mathrm e}$};
    % rotation carrying one to the other
    \draw[->, gray!70!black, line width=0.8pt]
         (0.98,1.78) to[bend right=30] (0.72,1.90);
    \node[gray!55!black, above, inner sep=0.5pt] at (0.86,1.99) {$R(q)$};
  \end{tikzpicture}
  \caption{The phone \emph{records} $\mathbf a=(a_x,a_y,a_z)$ in its
    tilted body frame; we \emph{want} the earth-frame
    $\mathbf a^{\mathrm e}=(a_x,a_y,a_z)_{\mathrm e}=R(q)\,\mathbf a$,
    whose vertical channel is the ride signal.}
  \label{fig:accel-frame}
  \vspace{-0.5em}
\end{wrapfigure}

The pipeline reads two of the phone's inertial sensors --- the
accelerometer and the gyroscope --- and one obstacle separates their
raw signal from the vertical acceleration a ride produces: the phone's
unknown, time-varying orientation.
...
body direction points up at every instant.  Gravity in the stationary
windows around a ride fixes that direction only at rest; a hand carried
through a ride rotates, so the direction must be tracked through the
ride itself.

\paragraph{Reconstructing the world frame.}
A Mahony passive complementary filter~\cite{mahony2008} tracks the
body-to-world orientation, seeded from the opening stationary window.
The orientation is a rotation matrix~$R\in SO(3)$ ($R^{\!\top}R=I$,
$\det R=1$) that maps body-frame vectors to the world frame; we carry it
as a unit quaternion~$q=(\cos\tfrac\theta2,\ \hat{\mathbf n}\sin\tfrac
\theta2)$, a four-number, singularity-free encoding of a rotation by
angle~$\theta$ (radians) about the unit axis~$\hat{\mathbf n}$, and write
the induced matrix~$R(q)$.  The gyroscope~$\boldsymbol\omega$
propagates~$q$ and the accelerometer (gravity retained,
$\hat{\mathbf a}=\mathbf a/\lVert\mathbf a\rVert$) corrects it toward
vertical.
\begin{gather}
  \hat{\mathbf v}=R(q)^{\!\top}\hat z_w,\qquad
  \mathbf e=\hat{\mathbf a}\times\hat{\mathbf v},
  \label{eq:mahony-err}\\
  w=\exp\!\Bigl(-\tfrac12\bigl((\lVert\mathbf a\rVert-g)/\tau g\bigr)^{2}\Bigr),
  \qquad
  \dot{\mathbf b}=-k_I\,w\,\mathbf e,
  \label{eq:mahony-gate}\\
  \dot q=\tfrac12\,q\otimes
         \bigl[\,0,\;\boldsymbol\omega-\mathbf b+k_P\,w\,\mathbf e\,\bigr]^{\!\top},
  \label{eq:mahony}\\
  \mathbf a^{\mathrm e}=\bigl(a^{\mathrm e}_x,a^{\mathrm e}_y,a^{\mathrm e}_z\bigr)
    =R(q)\,\mathbf a,\qquad a=a^{\mathrm e}_z-g .
  \label{eq:world-accel}
\end{gather}
The proportional term~$k_P w\mathbf e$ pulls~$q$ toward gravity, the
integral term learns the gyro bias~$\mathbf b$, and the trust
weight~$w\!\to\!0$ freezes the correction when
$\lVert\mathbf a\rVert\!\neq\!g$, so ride acceleration is never read as
tilt ($k_P\!=\!1,\,k_I\!=\!0.3$).  With no gyroscope, $q$ holds at its
seed and the vertical channel falls back to the rest-frame
magnitude~$\lVert\mathbf a\rVert-g$, which the mid-ride tilt corrupts.
Tracking~$q$ instead cuts the per-ride error close to threefold overall
--- halving the mean~$\Delta h$ error and shrinking the RMSE tail
$4$--$6\times$ (Appendix~\ref{app:eval-reconstruct}).
...
\caption{Predicted versus true~$\Delta h$ for the trapezoid fit on
  every clean ride, pooled across train and test ($n\!=\!586$).
  Points hug the $y\!=\!x$ diagonal across roughly $50$\,m of
  $\Delta h$; green points pass the quality filter, red crosses are
  rejected, and the dotted lines mark the $\pm1.5$\,m one-floor band.
...
Trapezoid & Test  & $\phantom{0}91$ & $84\%$ & $0.22$ & $0.38$ & $97.8\%$ & $96.7\%$ \\
...
\caption{Deployed pipeline on accepted predictions only
  ...
  trims the accepted-ride mean error --- to $0.38\,$m for the
  trapezoid fit and $0.47\,$m for ZUPT on the held-out test ---
  lifts the one-floor-target rate to $94\text{--}98\,\%$ there, and
  holds realised coverage at or above the nominal~$90\,\%$ on every
  row.  Every false positive on the held-out test ($9$ of $108$
  segmenter predictions) is rejected by the filter; the consumer
  sees zero phantom rides on \texttt{Beit Yitzchaki~B}.}
...
reaches $96.7\,\%$ at a nominal $90\,\%$, and the quality filter trims
the accepted-ride trapezoid mean error to $0.38\,$m on the test split
...
\subsection{Gyroscope reconstruction ablation}
\label{app:eval-reconstruct}

Table~\ref{tab:eval-reconstruct} isolates the gain from the earth-frame
reconstruction of~\S\ref{sec:bg-imu}: it scores both predictors on the
same pooled clean rides with the gyroscope off (\texttt{none}, the fixed
rest-frame gravity projection) and on (\texttt{Mahony}).  Reconstruction
cuts the mean~$\Delta h$ error $1.8$--$2.3\times$ and the outlier-driven
RMSE $4$--$6\times$; the RMSE gain is larger because the worst damage
without it falls on the few heavily rotated rides, whose fixed-gravity
tilt bias runs to tens of metres.  The one-floor rate rises on both
predictors, most for ZUPT, whose double integration compounds any leaked
tilt.
...
\toprule
Algorithm & Reconstruction & MAE (m) & RMSE (m) & $\le 1.5$\,m & $\le 3$\,m \\
\midrule
ZUPT      & \texttt{none} (baseline) & $2.38$ & $19.14$ & $77.6\%$ & $88.9\%$ \\
ZUPT      & \textbf{Mahony}          & $\mathbf{1.05}$ & $\mathbf{3.28}$ & $\mathbf{87.9\%}$ & $\mathbf{92.5\%}$ \\
\midrule
Trapezoid & \texttt{none} (baseline) & $1.61$ & $11.49$ & $87.5\%$ & $91.3\%$ \\
Trapezoid & \textbf{Mahony}          & $\mathbf{0.91}$ & $\mathbf{2.76}$ & $\mathbf{89.4\%}$ & $\mathbf{93.9\%}$ \\
\bottomrule
\end{tabular}
\caption{Prediction accuracy without (\texttt{none}) and with
  (\texttt{Mahony}) the accel+gyro earth-frame reconstruction, pooled
  across train and test ($586$ clean rides).  Reconstruction cuts the
  mean error $1.8$--$2.3\times$ and the RMSE tail $4$--$6\times$ (ZUPT
  $2.38\!\to\!1.05$\,m MAE, $19.1\!\to\!3.3$\,m RMSE; trapezoid
  $1.61\!\to\!0.91$\,m, $11.5\!\to\!2.8$\,m); the Mahony CIs stay
  conformally calibrated to $90\,\%$ (Table~\ref{tab:eval-pred}).}
\label{tab:eval-reconstruct}
\end{table}
...
\caption{Empirical CDF of absolute $\Delta h$ error for the
  trapezoid fit on clean rides (all rides, red; quality-accepted,
  green).  Vertical dashed lines mark the $\pm 1.5$\,m and
  $\pm 3$\,m floor targets; the all-ride curve crosses $\pm 1.5$\,m
  by roughly the $89\,\%$ percentile and $\pm 3$\,m by the
  $94\,\%$ percentile, while the accepted curve clears $\pm 1.5$\,m
  by the $97\,\%$ percentile.}
