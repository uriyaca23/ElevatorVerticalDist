...
def _run_on_experiments(
    ...
    for name in experiments:
        ...
        # Pass the gyroscope so the detector can reconstruct orientation when
        # config.reconstruct != "none" (ignored otherwise).
        segments = segmenter.detect(
            data, phone_model=phone, gyro=sensors.get("GYR"),
        )
