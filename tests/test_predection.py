...
def _first_segments(acc, n=5):
    ...
    # the prediction pins below compare against the shared core, which runs on
    # the same raw acc. The resample path is covered in test_api_resampling.py.
...
def test_predictSegment_matches_core(acc):
    for seg in _first_segments(acc):
        ...
        for aid in rows_by_algo:
            deep_equal(got.row(aid).model_dump(), rows_by_algo[aid][0],
                       path=aid)
...
def test_predictSegment_subset_algorithms(acc):
    ...
    assert got.trap is not None
    assert got.zupt is None and got.baro is None
    rows_by_algo, _ = _core_predict(acc, [seg], algorithms=["trap"])
    deep_equal(got.trap.model_dump(), rows_by_algo["trap"][0])
...
def test_predictByParameters_matches_override(acc):
    ...
    assert got.primary == primary
    for aid in rows_by_algo:
        deep_equal(got.row(aid).model_dump(), rows_by_algo[aid][0], path=aid)
