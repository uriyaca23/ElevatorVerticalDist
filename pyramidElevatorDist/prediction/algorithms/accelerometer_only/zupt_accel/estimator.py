...
from pyramidElevatorDist.utils.signal_processing import butter_lowpass
...
class ZuptAccelEstimator:
    ...
    # ------------------------------------------------------------------
    # Committed conformal calibration for this algorithm, living next to
    # the code as an algorithm "setting". Auto-loaded at construction so
    # every prediction is calibrated with no external wiring, and
    # overwritten in place by ``evaluateOnData --calibrate``.
    ...
    def load(self, path: Path | str | None = None) -> None:
        self.conformal = ConformalCalibrator.load(path or self.CALIBRATION_PATH)
