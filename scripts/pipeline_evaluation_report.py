...
def write_macros(
    ...
    def block(prefix: str, src: dict, acc: bool = False) -> list[str]:
        ...
        for view in ("gt", "matched", "all"):
            s = src[view]["summary"]
            tag = view.capitalize()
            ll.append(f"\\newcommand{{\\Pipe{prefix}{tag}N{suffix}}}{{{s['n']}}}")
            ll.append(f"\\newcommand{{\\Pipe{prefix}{tag}Mae{suffix}}}{{{s['mae']:.2f}}}")
            ll.append(f"\\newcommand{{\\Pipe{prefix}{tag}Median{suffix}}}{{{s['median']:.2f}}}")
            ll.append(f"\\newcommand{{\\Pipe{prefix}{tag}Rmse{suffix}}}{{{s['rmse']:.2f}}}")
            ll.append(f"\\newcommand{{\\Pipe{prefix}{tag}Half{suffix}}}"
                      f"{{{100*s['p_within_0_5m']:.1f}\\%}}")
            ll.append(f"\\newcommand{{\\Pipe{prefix}{tag}Floor{suffix}}}"
    ...
    lines.append(f"\\newcommand{{\\PipeFpN}}{{{n_fp}}}")
    lines.append(f"\\newcommand{{\\PipeFpMedianSigned}}{{{fp_median_signed:+.2f}}}")
    lines.append(f"\\newcommand{{\\PipeFpMedianAbs}}{{{fp_median_abs:.2f}}}")
    lines.append(f"\\newcommand{{\\PipeFpMeanAbs}}{{{fp_mean_abs:.2f}}}")
