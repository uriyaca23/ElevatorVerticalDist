...
class PREDICT_ALGORITHM_CONFIG(BaseModel):
    ...
    # accelerometer algorithms first resample the ride window and its pre/post
    # gravity-calibration windows onto a uniform grid at this rate (gap-aware,
    # time-correct), so the Δh estimators — tuned for a fixed cadence — are
    # robust to data at any/variable sample rate. ``None`` (default) = consume
    # the windows as-is (native cadence); keeps the research evaluators and
    # their reported numbers unchanged. The boutique pipeline sets it to 50.
