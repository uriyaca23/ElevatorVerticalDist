...
``{timestamp}_segmentation_{kind}/`` under ``--out-root`` (default
``elevator_reports``). Every
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Accel+gyro orientation reconstruction applied as the first "
             "step of the ACC detector (default: none = raw accelerometer). "
             "Other values feed a virtually-flat-phone accel to the detector.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Base directory; the run is written to "
             "<out-root>/<timestamp>_segmentation_<kind>/.",
    )
    p.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    cfg = SEGMENT_ALGORITHM_CONFIG(
        algorithm=SegmentAlgorithm(args.algorithm),
    ...
    run_dir = args.out_root / (
        args.run_name or f"{timestamp}_segmentation_{args.kind}")
