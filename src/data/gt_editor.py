...
class GtEditor(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Navigation controls get their own full-width row *below* the top
        # bar so the zoom buttons never get pushed off-screen by the
        # combobox / save buttons on a narrow (MacBook) display. Row 0 =
        # zoom, row 1 = pan, columns aligned so each pan button sits
        # directly under its zoom button.
        nav_bar = ttk.Frame(self, padding=(6, 0, 6, 4))
        nav_bar.pack(fill=tk.X)
        nav = ttk.Frame(nav_bar)
        ...
        ttk.Label(edit, text="start (HH:MM:SS.mmm):").grid(row=0, column=0, sticky=tk.W)
        self.start_var = tk.StringVar()
        ttk.Entry(edit, textvariable=self.start_var, width=14)\
            .grid(row=0, column=1, padx=4, pady=2)

        ttk.Label(edit, text="end (HH:MM:SS.mmm):").grid(row=1, column=0, sticky=tk.W)
    ...
    def load_experiment(self):
        ...
        # Route the native ACC through the same shared loader the
        # segmentation editor and the Streamlit boutique pipeline use
        # (resample to 50 Hz, split per gap). Anchoring the time axis on the
        # *resampled* first sample — not the raw first sample — is what keeps
        # all three tools on one time domain. Without this, a leading sparse
        # region trimmed by gap detection shifts gt_editor's origin relative
        # to the other tools, and GT bands appear offset when reopened there.
        acc_native = sensors.get("ACC")
        if acc_native is not None and not acc_native.empty:
            base = LoadedSignal(
    ...
    def _ms_to_clock(self, ms: int | float) -> str:
        """Absolute epoch ms -> wall-clock ``HH:MM:SS.mmm`` for the edit
        form. Falls back to the raw ms string when no experiment anchor is
        available (``_t0_ms <= 0``)."""
        clock = self._wall_time_str((float(ms) - self._t0_ms) / 1000.0,
                                    sub_second=True)
        return clock or str(int(ms))

    def _clock_to_ms(self, text: str) -> int:
        """Parse ``HH:MM:SS[.mmm]`` back to absolute epoch ms, using the
        loaded experiment's start date as the reference day. Raises
        ``ValueError`` on malformed input.

        As a convenience (and to stay backward-compatible with older saved
        habits) a bare integer is accepted as raw epoch ms.
        """
        s = text.strip()
        if self._t0_ms <= 0:
            return int(s)
        ...
        if m is None:
            # Allow a raw-ms paste to still work.
            return int(s)
        hh, mm, ss = int(m.group(1)), int(m.group(2)), int(m.group(3))
        ms = int((m.group(4) or "0").ljust(3, "0"))
        ref = datetime.fromtimestamp(self._t0_ms / 1000.0)
        best: int | None = None
        for day_shift in (-1, 0, 1):
            cand = ref.replace(hour=hh, minute=mm, second=ss,
                               microsecond=ms * 1000) + timedelta(days=day_shift)
            cand_ms = int(round(cand.timestamp() * 1000.0))
            if best is None or abs(cand_ms - self._t0_ms) < abs(best - self._t0_ms):
                best = cand_ms
        return int(best)
    ...
    def _on_motion(self, event):
        ...
        if self._drag_edge == "start":
            ...
            if new_ms < end_ms - 10:
                ...
                self.start_var.set(self._ms_to_clock(new_ms))
        else:  # "end"
            start_ms = int(row["start_ms"])
            if new_ms > start_ms + 10:
                self.pipeline.gt.loc[self._drag_idx, "end_ms"] = new_ms
                self.end_var.set(self._ms_to_clock(new_ms))
    ...
    def _on_tree_select(self, _event=None):
        ...
        self.start_var.set(self._ms_to_clock(int(row["start_ms"])))
        self.end_var.set(self._ms_to_clock(int(row["end_ms"])))
    ...
    def _on_apply(self):
        ...
        idx = int(sel[0])
        try:
            start = self._clock_to_ms(self.start_var.get())
            end = self._clock_to_ms(self.end_var.get())
        except ValueError:
            messagebox.showerror(
                "Invalid", "start/end must be HH:MM:SS.mmm (or raw epoch ms).")
            return
