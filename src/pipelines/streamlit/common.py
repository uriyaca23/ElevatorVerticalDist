...
# Algorithm short ids consumed by ``pyramidElevatorDist`` predictions
# (``predictSegment`` / ``predictByParameters`` return one row per id). The
# boutique UI labels them and their colours below.
ALGO_TRAP = "trap"
ALGO_ZUPT = "zupt"
...
def init_state() -> None:
    defaults = {
        ...
        #   segment_params — {segment position: findSegmentParameters result}.
        "display_signal":     None,
        "segment_params":     {},
...
# Inverse helpers: convert between the relative-seconds the segments are
# *stored* in (the algorithm contract) and the wall-clock strings the user
# *edits*. The forward direction reuses to_datetime() so the clock readout
# shares the same origin and timezone as every chart.

_CLOCK_RE = re.compile(r"^\s*(\d{1,2}):(\d{2}):(\d{2})(?:\.(\d{1,3}))?\s*$")


def seconds_to_clock(
    t_s: float, t0_ms: float | None, *, sub_second: bool = True,
) -> str:
    """Relative seconds-from-start -> ``HH:MM:SS.mmm`` (or ``HH:MM:SS``).

    Inverse of :func:`clock_to_seconds`. With a missing / bad ``t0_ms`` the
    origin falls back to the Unix epoch (same graceful behaviour the charts
    have via :func:`_t0_ms`), so the readout never crashes.
    """
    ts = to_datetime(float(t_s), t0_ms)
    if sub_second:
        return ts.strftime("%H:%M:%S.") + f"{ts.microsecond // 1000:03d}"
    return ts.strftime("%H:%M:%S")


def clock_to_seconds(text: str, t0_ms: float | None) -> float:
    """``HH:MM:SS[.mmm]`` -> relative seconds-from-start.

    Inverse of :func:`seconds_to_clock`. The recording's start instant
    (``to_datetime(0.0, t0_ms)``) supplies the reference date; the parsed
    time-of-day is placed on that date and, to handle recordings that cross
    midnight, also on the adjacent days — whichever lands closest to the
    start is returned. Raises :class:`ValueError` on malformed input.
    """
    m = _CLOCK_RE.match(text or "")
    if m is None:
        raise ValueError(f"expected HH:MM:SS[.mmm], got {text!r}")
    hh, mm, ss = int(m.group(1)), int(m.group(2)), int(m.group(3))
    micro = int((m.group(4) or "0").ljust(3, "0")) * 1000
    if hh > 23 or mm > 59 or ss > 59:
        raise ValueError(f"time field out of range: {text!r}")
    ref = to_datetime(0.0, t0_ms).to_pydatetime()
    best: float | None = None
    for day_shift in (-1, 0, 1):
        cand = ref.replace(
            hour=hh, minute=mm, second=ss, microsecond=micro,
        ) + timedelta(days=day_shift)
        delta = (cand - ref).total_seconds()
        if best is None or abs(delta) < abs(best):
            best = delta
    return float(best)


def _current_t0_ms() -> float | None:
    """The session's recording origin (epoch ms), or ``None`` before a signal
    is loaded. It is simply the first accelerometer timestamp of the loaded
    (already 50 Hz-resampled) trace. Shared by the clock-time edit widgets."""
    loaded = st.session_state.get("loaded")
    if loaded is None or loaded.acc is None or loaded.acc.empty:
        return None
    return float(loaded.acc["timestamp_ms"].iloc[0])
...
def reset_downstream_state() -> None:
    ...
    st.session_state["display_signal"] = None
...
def _segment_label(i: int, row: pd.Series, has_override: bool = False) -> str:
    ...
    # Show the wall-clock start so the list reads the same way as the charts
    # and the edit form; keep the duration in seconds (it's the natural unit
    # for a ride length).
    t0 = _current_t0_ms()
    return (f"#{i:<2} {star}{rt:<4}  {seconds_to_clock(s, t0, sub_second=False)}"
            f"  ({e - s:.1f} s)")
...
def _render_add_segment_controls(segments_df: pd.DataFrame) -> None:
    ...
    if st.session_state.get("pending_new_segment"):
        ...
        st.sidebar.caption("Times are wall-clock HH:MM:SS.mmm.")
        new_lo = st.sidebar.text_input(
            "Start", value=seconds_to_clock(default_lo, t0),
            key="sb_new_lo",
        )
        new_hi = st.sidebar.text_input(
            "End", value=seconds_to_clock(default_hi, t0),
            key="sb_new_hi",
        )
        ...
        if accept:
            try:
                lo_s = clock_to_seconds(new_lo, t0)
                hi_s = clock_to_seconds(new_hi, t0)
            except ValueError:
                st.sidebar.error("Use clock time HH:MM:SS.mmm.")
            else:
                if hi_s <= lo_s:
    ...
    if len(segments_df) == 0:
        ...
        with col_edit:
            with st.popover("✎  Edit selected", use_container_width=True):
                t0 = _current_t0_ms()
                st.caption("Times are wall-clock HH:MM:SS.mmm.")
                new_lo = st.text_input(
                    "Start", value=seconds_to_clock(float(row["start_s"]), t0),
                    key=f"sb_edit_lo_{sel}",
                )
                new_hi = st.text_input(
                    "End", value=seconds_to_clock(float(row["end_s"]), t0),
                    key=f"sb_edit_hi_{sel}",
                )
                ...
                if st.button("Save", key=f"sb_edit_save_{sel}",
                    ...
                    try:
                        ...
                        st.error("Use clock time HH:MM:SS.mmm.")
                    else:
                        if hi_s <= lo_s:
                            st.error("End must be later than start.")
                        else:
                            df = st.session_state["segments_df"]
                            df.loc[df.index[sel], "start_s"] = float(lo_s)
                            df.loc[df.index[sel], "end_s"] = float(hi_s)
