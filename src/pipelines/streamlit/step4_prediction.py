...
def _run_predictions(
    ...
    Calls ``predictSegment`` once per segment (``predictByParameters`` when a
    manual trapezoid override is staged). The package owns the per-segment
    slicing and the gravity pre/post window logic — the UI just hands over the
    full session ACC (already 50 Hz, so ``resample=False``) and each segment.
    Reconstruction changes the ZUPT (a_vert) Δh, so ``gyro`` +
    ``reconstruct`` are passed through. Defensive: any segment whose span
    crosses a "no data" gap is dropped before predicting (the user is shown
    the red gap overlays in step 3, but we filter here too).

    Each row's ``segment`` is set to its position in ``segments`` so the rest
    of step 4 keys on a stable index.
    """
    ...
    for pos, (_, row) in enumerate(segments.iterrows()):
        ...
        if ov is not None:
            result = predictByParameters(
                loaded.acc, seg,
                {"W": ov["W"], "f": ov["f"], "abs_A": ov["abs_A"]},
                resample=False, gyro=loaded.gyr, reconstruct=method,
            )
        else:
            result = predictSegment(
                loaded.acc, seg,
                resample=False, gyro=loaded.gyr, reconstruct=method,
            )
...
def _repredict_single_segment(
    ...
    if override is not None:
        result = predictByParameters(
            loaded.acc, seg,
            {"W": override["W"], "f": override["f"], "abs_A": override["abs_A"]},
            resample=False, gyro=loaded.gyr, reconstruct=method,
        )
    else:
        result = predictSegment(
...
def render() -> None:
    ...
    if loaded is None or segments is None:
    ...
    # Overview display signal — reuse the one step 3 cached, recomputing only
    # if it is missing (e.g. deep-linked). Reconstruction changes a_vert.
    ...
    st.plotly_chart(
        _prediction_bar_figure(rows_by_algo, selected, t0_ms),
