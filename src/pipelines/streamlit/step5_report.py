...
from .step3_segmentation import _seg_key
...
def _build_correlation_png(
    ...
    mask = (t >= wlo) & (t <= whi)
    dt_corr = to_datetime_array(t[mask], t0_ms)

    fig, ax = plt.subplots(figsize=(7.6, 2.7), dpi=180)
    fig.patch.set_facecolor("white")
    ax.plot(dt_corr, pos_plot[mask], color="#2980b9", lw=0.9,
            label="max R² (+)")
    ax.plot(dt_corr, neg_plot[mask], color="#c0392b", lw=0.9,
            label="max R² (−)")
    ax.axvspan(to_datetime(t_lo, t0_ms), to_datetime(t_hi, t0_ms),
               color=SELECTED_COLOR, alpha=0.10, lw=0)
    ax.set_ylim(0, 1.05)
    ax.set_xlim(to_datetime(wlo, t0_ms), to_datetime(whi, t0_ms))
    loc = AutoDateLocator()
    ax.xaxis.set_major_locator(loc)
    ax.xaxis.set_major_formatter(AutoDateFormatter(loc))
    ax.set_xlabel("time"); ax.set_ylabel("R² (per sign)")
    ax.legend(loc="upper right", fontsize=7, frameon=False, ncol=2)
    _style_axes(ax)
    fig.autofmt_xdate()
    fig.tight_layout()
...
def _correlation_for_report(
    ...
    ``findSegmentParameters`` result cached by step 3 when available, and
    otherwise recompute once for the first segment. Segmentation is
    magnitude-invariant, so this call stays acc-only.
    """
    cache = st.session_state.get("segment_params") or {}
    for pos in range(len(segments)):
        row = segments.iloc[pos]
        try:
            t_lo = float(row["start_s"]); t_hi = float(row["end_s"])
...
def render() -> None:
    ...
    st.markdown(
        ...
        '<p>One page per segment: the ±30 s correlation window, the '
        'trapezoid fit and ZUPT trajectory, and a segment summary.</p>'
        '</div>',
        unsafe_allow_html=True,
    )

    method = st.session_state.get("reconstruct", "none")
    t0_ms = (
        float(loaded.acc["timestamp_ms"].iloc[0])
        if not loaded.acc.empty else 0.0
    )
    # Presentation data: the display signal cached in step 3 and the global
    # correlation curves. Fall back to recomputing from the package if a
    # cache entry is missing (e.g. the user deep-linked into step 5).
    disp = st.session_state.get("display_signal")
    if disp is None or getattr(disp, "empty", True):
        disp = reconstructedSignal(
            loaded.acc, gyro=loaded.gyr, reconstruct=method, resample=False,
        )
    correlation = _correlation_for_report(loaded, segments)
