...
A self-describing ``{timestamp}_pipeline_{kind}/`` directory is created
under ``--out-root`` (default ``elevator_reports``):
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Optional conformal-calibration JSON to load onto the "
             "predictor. Default: none — the predictor auto-loads the "
             "prediction algorithm's own committed calibration.json.",
    )
    p.add_argument(
        "--out-root", type=Path, default=DEFAULT_OUT_ROOT,
        help="Base directory; output → "
             "<out-root>/<timestamp>_pipeline_<kind>/.",
    )
    p.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_dir = args.out_root / (
        args.run_name or f"{timestamp}_pipeline_{args.kind}")
