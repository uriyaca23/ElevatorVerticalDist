...
class SegmentRecord:
    ...
    # Optional gyroscope slices for the same ride / pre / post windows, used
    # when the predictor's ``reconstruct`` is set. ``None`` when the
    # experiment logged no gyroscope.
    gyr: Optional[pd.DataFrame] = field(default=None, repr=False)
    pre_gyr: Optional[pd.DataFrame] = field(default=None, repr=False)
    post_gyr: Optional[pd.DataFrame] = field(default=None, repr=False)
...
def build_segment_records(
    ...
    acc = sensors["ACC"]
    # Gyroscope for the same windows (carried raw; reconstruction, if any,
    # happens inside Predictor.predict). None when the phone logged no gyro.
    gyr = sensors.get("GYR")
    ...
    for idx, row in gt.iterrows():
        ...
        ride_gyr = _slice_acc(gyr, s, e) if gyr is not None else None
        pre_gyr = _slice_acc(gyr, s - pre_ms, s) if gyr is not None else None
        post_gyr = _slice_acc(gyr, e, e + post_ms) if gyr is not None else None
        ...
        rec = SegmentRecord(
            ...
            gyr=ride_gyr, pre_gyr=pre_gyr, post_gyr=post_gyr,
        )
        out.append(rec)
    return out
