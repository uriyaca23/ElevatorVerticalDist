...
def _first_segments(acc, n=5):
    ...
    # the prediction pins below compare against api_client, which runs on the
    # same raw acc. The resample path is covered in test_api_resampling.py.
...
def test_predictSegment_matches_api_client(acc):
    for seg in _first_segments(acc):
        got = predictSegment(acc, seg, resample=False)
        rows_by_algo, primary = api_client.predict(acc, [seg])
...
def test_predictSegment_subset_algorithms(acc):
    ...
    rows_by_algo, _ = api_client.predict(acc, [seg], algorithms=["trap"])
...
def test_predictByParameters_matches_override(acc):
    ...
    rows_by_algo, primary = api_client.predict(acc, [seg_override])
