...
def _current_t0_ms() -> float | None:
    """The session's recording origin (epoch ms), or ``None`` before the
    detector has run. Shared by the clock-time edit widgets."""
    state = st.session_state.get("detector_state") or {}
    return state.get("t0_ms")
